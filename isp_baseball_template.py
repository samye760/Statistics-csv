"""
  This module utilizes Python's built-in csv module to computer
  numerous different statistics.
"""

import csv

##
## Python csv module DictReader code
##
...
##
## Formulas for common batting statistics
##
...
def filter_by_year(statistics, year, yearid):
    ...
    for statistic in statistics:
        if int(statistic[yearid]) == year:
            statistics_by_year.append(statistic)
          
    return statistics_by_year
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    formula_result = []
    for statistic in statistics:
        formula_result.append( (statistic[info["playerid"]], formula(info, statistic)) )
    
    return sorted(formula_result, key = lambda tup: tup[1], reverse = True)[:numplayers]
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    for stat in top_ids_and_stats:
        names.append("{:.3f} --- {} {}".format(
          stat[1], read_csv_as_nested_dict(
            info["masterfile"], info["playerid"], info["separator"], 
            info["quote"])[stat[0]][info["firstname"]], 
          read_csv_as_nested_dict(info["masterfile"], info["playerid"], 
                                  info["separator"], info["quote"])[stat[0]][info["lastname"]]))
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    """
    year_dict = filter_by_year(read_csv_as_list_dict(
      info["battingfile"], info["separator"], info["quote"]), year, info["yearid"])
    
    return lookup_player_names(info, top_player_ids(info, year_dict, formula, numplayers))
##
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    for player in statistics:
        if player[playerid] not in players_agg:
            players_agg[player[playerid]] = ( {playerid: player[playerid]} )
            players_agg[player[playerid]].update( { stat: player[stat] for stat in fields } )
            for field in fields:
                players_agg[player[playerid]][field] = int(players_agg[player[playerid]][field])
            
        else:
            for field in fields:
                players_agg[player[playerid]][field] = \
                players_agg[player[playerid]][field] + int(player[field])
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    Inputs:
      ...
      numplayers  - Number of top players to return
    """
    agg_career = aggregate_by_player_id(read_csv_as_list_dict(
      info["battingfile"], info["separator"], info["quote"]), 
                                        info["playerid"], info["battingfields"])
    
    career_list = list(agg_career.values())
    
    return lookup_player_names(info, top_player_ids(info, career_list, formula, numplayers))
